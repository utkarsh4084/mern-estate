...
export default function UpdateListing() {
    ...
    const handleRemoveImage = (index) => {
        setFormData({...formData, imageUrls: formData.imageUrls.filter((_, i) => index !== i)})
    }
    ...
    const handleChange = (e) => {
        ...
        if(e.taget.id === 'furnished' || e.target.id === 'parking' || e.target.id === 'offer') {
            setFormData({
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if(formData.imageUrls.length < 1) setError('You must upload atleast 1 image')
            if(+formData.regularPrice < +formData.discountedPrice) return setError('Discount Price must be less than regular')
            setLoading(true)
            setError(false)
        ...
        <main className="p3 max-w-4xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
                <div className="flex flex-col gap-4 flex-1">
                    <input type='text' placeholder="Name" id="name" minLength="10" maxLength="62" required 
                        onChange={handleChange} value={formData.name} className="border p-3 rounded-lg"/>
                    <textarea type="text" placeholder="Description" id="required" required 
                    ...
                    <div className="flex flex-wrap gap-6">
                        ...
                        </div>
                        {
                            formData.offer && 
                            (<div className='flex items-center gap-2'>
                                <input type="number" id="discountPrice" min="50" max="10000000" required
                    ...
                    <div className='flex flex-col flex-1 gap-4'>
                        ...
                        <div className='flex gap-4'>
                            <input type="file" id="images" accept="image/*" multiple 
                                onChange={(e) => setFiles(e.target.files)} className='p-3 border border-gray-300 rounded w-full'/>
